...
const openGROUPS = async () => {
    const data = await fs.promises.readFile(groupsPath, 'utf-8');
    return JSON.parse(data);
}
...
core.post("/api/set-online", async(req, res) => {
    ...
    if(users[userID]) {
        ...
        users[userID].statusNetwork = true;
        await saveUSERS(users);
...
core.post("/api/set-ofline", async (req, res) => {
    ...
    if(users[userID]) {
        ...
        users[userID].statusNetwork = false;
        await saveUSERS(users);
...
core.post("/api/get-all-message", async (req, res) => {
    const data = req.body
    const messages = await openROOMS()

    const send = messages[data.idRoom]?.messages || [];
...
core.post("/api/create-groups", async (req, res) => {
    ...
    const newGroup = new Group(data.id, data.users);
...
core.post("/api/blocked-user", async (req, res) => {
    ...
    if (!user.blockedUser.includes(data.blockedUser)) {
        user.blockedUser.push(data.blockedUser);
    }
    await saveUSERS(users);
...
core.post("/api/unblocked-user", async (req,res) => {
    ...
    users[data.userID].blockedUser = users[data.userID].blockedUser.filter(user => user !== data.userBlocked);
    await saveUSERS(users);
...
core.post("/api/create-channel", async (req,res) => {
    ...
    const newChannel = new Channel(data.idChannel, data.owners)

    await newChannel.createChannel()
